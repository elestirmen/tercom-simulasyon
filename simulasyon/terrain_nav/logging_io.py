...
def save_results(results: List[Tuple[Tuple[float, float, float], EstimatedState]], filepath: str):
    ...
    with open(filepath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            "timestamp_s", "true_x", "true_y", "true_heading", 
            "est_x", "est_y", "est_heading", "est_msl", 
            "error_x", "error_y", "error_pos", "error_heading",
            "is_ambiguous", "score"
        ])
        ...
        for true_s, est_s in results:
            ...
            writer.writerow([
                est_s.timestamp_s, true_s[0], true_s[1], true_s[2],
                est_s.estimated_x, est_s.estimated_y, est_s.estimated_heading_deg, est_s.estimated_msl_m,
                err_x, err_y, err_pos, err_h,
                int(est_s.is_ambiguous), est_s.score
            ])
