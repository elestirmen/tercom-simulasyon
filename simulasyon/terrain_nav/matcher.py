...
def evaluate_candidate(
    ...
    rmse = float(np.sqrt(np.mean(error**2))) if compute_all_metrics or loss_method == "rmse" else 0.0
...
class ProfileMatcher:
    ...
    def _vectorized_known_altitude_search(
        ...
        for measurement_index, measurement_valid in enumerate(laser_valid):
            ...
            if loss_method == "mae":
                ...
                loss = 0.5 * quadratic**2 + huber_delta * (
                    absolute_error - quadratic
                )
            else:
                loss = error**2
    ...
    def coarse_to_fine_search(
        ...
        # 1. Coarse search
        if (
            self.config.sensor.altitude_mode == "known_msl_altitude"
            and len(search_headings) == 1
        ):
        ...
        for c in coarse_cands:
            ...
            for r in range(r_start, r_end, m_stride):
                for col in range(c_start, c_end, m_stride):
                    cand = evaluate_candidate(
                        r, col, c.heading_deg, self.dem, laser_agl, laser_valid, baro_msl,
                        base_offsets, self.ct, self.config,
                        pixel_offsets=self._pixel_offsets_for_heading(
                    ...
                    if cand is not None:
                        self._retain_top_candidate(
                            medium_heap, cand, medium_sequence, top_k
                        )
                        medium_sequence += 1
            ...
            for r in range(r_start, r_end, f_stride):
                for col in range(c_start, c_end, f_stride):
                    for h in fine_headings:
                        cand = evaluate_candidate(
                            r, col, h, self.dem, laser_agl, laser_valid, baro_msl,
                            base_offsets, self.ct, self.config,
                            pixel_offsets=self._pixel_offsets_for_heading(
                                pixel_offset_cache, base_offsets, h
                            ),
