"""Tests for bounded external GeoTIFF DEM loading."""
...
def test_external_dem_reads_requested_window(tmp_path):
    ...
    np.testing.assert_array_equal(terrain.get_truth_dem(), source[2:6, 3:7])
    np.testing.assert_array_equal(terrain.get_navigation_dem(), source[2:6, 3:7])
    assert np.isclose(terrain.dx, 0.3)
    assert np.isclose(terrain.dy, 0.3)
    assert np.allclose(terrain.get_extent(), (1.2, 1.2))
    assert terrain.window_offset == (2, 3)
    ...
    assert np.allclose(terrain.get_display_offset(), (500000.9, 4199999.4))
    assert np.allclose(
        terrain.get_navigation_bounds_on_display(),
        (500000.9, 500002.1, 4199998.2, 4199999.4),
    )
